<template>
  ...
  <div v-if="doc.name" class="flex h-full overflow-hidden">
    ...
    <Resizer class="flex flex-col justify-between border-l" side="right">
      ...
      <!-- Tags Section -->
      <div class="border-b p-5 flex flex-col gap-2.5">
        <div class="text-sm font-medium text-ink-gray-5">{{ __('Tags') }}</div>
        <div class="flex flex-wrap gap-1.5 items-center">
          <Badge
            v-for="tag in getTags(doc._user_tags)"
            :key="tag"
            variant="subtle"
            theme="gray"
            size="md"
          ...
          >
            <template #default>
              <div class="flex items-center">
                <span>{{ tag }}</span>
                <button 
                  class="hover:text-red-500 ml-1.5 font-bold cursor-pointer focus:outline-none"
          ...
          <Popover>
            ...
            <template #body="{ close }">
              <div class="p-3 bg-surface-elevation-2 rounded-lg shadow-xl ring-1 ring-black ring-opacity-5 w-48 flex flex-col gap-2">
                <TextInput
                  v-model="newTag"
                ...
                <Button
                  ...
                  size="sm"
                  :label="__('Add')"
                  @click="() => { addTag(newTag); newTag = ''; close(); }"
                />
              </div>
            </template>
          </Popover>
...
async function removeTag(tag) {
  try {
    ...
    toast.error(__('Error removing tag'))
  }
}
